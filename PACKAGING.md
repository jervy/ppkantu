...
- 解决方案：`LiteImageViewer.sln`
- 主项目：`LiteImageViewer/LiteImageViewer.csproj`
...
```text
artifacts/
  bin/LiteImageViewer/<Debug|Release>/...
  obj/LiteImageViewer/<Debug|Release>/...
  publish/LiteImageViewer/Release/win-x64/
  publish/LiteImageViewer/Release/win-x64-single/
```
...
```bash
dotnet publish LiteImageViewer/LiteImageViewer.csproj -c Release -r win-x64 --self-contained false -v minimal
```
...
```text
D:\work\kantu\artifacts\publish\LiteImageViewer\Release\win-x64\
```
...
LiteImageViewer.exe
LiteImageViewer.dll
LiteImageViewer.deps.json
LiteImageViewer.runtimeconfig.json
相关依赖 dll
```
...
```bash
dotnet publish LiteImageViewer/LiteImageViewer.csproj -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -v minimal
```
...
```text
D:\work\kantu\artifacts\publish\LiteImageViewer\Release\win-x64-single\
```

单文件版主程序必须是：

```text
artifacts/publish/LiteImageViewer/Release/win-x64-single/LiteImageViewer.exe
```

Windows 路径为：

```text
D:\work\kantu\artifacts\publish\LiteImageViewer\Release\win-x64-single\LiteImageViewer.exe
```
...
注意：不要把运行时安装包打进 `LiteImageViewer.exe`。它只是放在同一目录，方便分发。
...
```text
LiteImageViewer/LiteImageViewer.csproj
...
```text
NETSDK1004: 找不到资产文件 "...\artifacts\obj\LiteImageViewer_xxxxx_wpftmp\project.assets.json"。
...
cd /d/work/kantu
rm -rf artifacts/obj
dotnet restore LiteImageViewer.sln -v minimal
dotnet build LiteImageViewer.sln -c Release -v minimal --no-restore
```
...
# 构建验证
dotnet build LiteImageViewer.sln -c Release -v minimal

# 主应用：普通目录版（浮窗子应用同步发布到 FloatingImageViewerPublish/）
dotnet publish LiteImageViewer/LiteImageViewer.csproj -c Release -r win-x64 --self-contained false -v minimal
dotnet publish FloatingImageViewerApp/FloatingImageViewerApp.csproj -c Release -r win-x64 --self-contained false -p:PublishDir="D:\work\kantu\artifacts\publish\LiteImageViewer\Release\win-x64\FloatingImageViewerPublish" -v minimal
...
> - **单文件版**：`win-x64-single/` 下只有 `LiteImageViewer.exe` + 运行时安装包，无需附带 `FloatingImageViewerPublish/` 子目录。浮窗 exe 已嵌入主 exe，首次使用"固定到桌面"时自动解压到 `%TEMP%/FloatingImageViewerPublish/`。
...
find artifacts/publish/LiteImageViewer/Release -maxdepth 3 \( -iname '*.pdb' -o -iname '*.cmd' -o -iname '*tmp*' -o -iname '*wpftmp*' \) -print

find artifacts/publish/LiteImageViewer/Release/win-x64-single -maxdepth 1 -type f -printf '%f\n' | sort
```
...
```bash
timeout 5s /d/work/kantu/artifacts/publish/LiteImageViewer/Release/win-x64-single/LiteImageViewer.exe; printf 'single_exit=%s\n' $?
```
