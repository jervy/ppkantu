...
artifacts/
  ...
  publish/ppkantu/Release/win-x64/
  publish/ppkantu/Release/win-x64-single/
```
...
```text
D:\work\kantu\artifacts\publish\ppkantu\Release\win-x64\
```
...
```text
D:\work\kantu\artifacts\publish\ppkantu\Release\win-x64-single\
```
...
dotnet publish ppkantu/ppkantu.csproj -c Release -r win-x64 --self-contained false -v minimal
dotnet publish FloatingImageViewerApp/FloatingImageViewerApp.csproj -c Release -r win-x64 --self-contained false -p:PublishDir="D:\work\kantu\artifacts\publish\ppkantu\Release\win-x64\FloatingImageViewerPublish" -v minimal
...
find artifacts/publish/ppkantu/Release -maxdepth 3 \( -iname '*.pdb' -o -iname '*.cmd' -o -iname '*tmp*' -o -iname '*wpftmp*' \) -print

find artifacts/publish/ppkantu/Release/win-x64-single -maxdepth 1 -type f -printf '%f\n' | sort
```
